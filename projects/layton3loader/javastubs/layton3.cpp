...
using namespace jnivm::com::Level5::LT3R;
...
{
    return std::make_shared<FakeJni::JString>("1.0.3");
}

// The Vita/Switch ports map to the engine's numeric language id:
// 1=EN, 2=ES, 3=FR, 4=IT, 5=DE (reference/layton2_nx-main/source/jni.c
// device_language(), unchanged in layton3_nx-main). Anything else --
// including an unset config -- falls back to English, same as the Switch
// port's default case.
int MainActivity::OS_GetDeviceLanguage()
{
    std::string lang = config["device"]["language"].value_or<std::string>("en");
    if (lang == "es") return 2;
    if (lang == "fr") return 3;
    if (lang == "it") return 4;
    if (lang == "de") return 5;
...
BEGIN_NATIVE_DESCRIPTOR(MainActivity){FakeJni::Constructor<MainActivity>{}},
    ...
    {FakeJni::Function<&MainActivity::OS_GetDeviceLanguage>{}, "OS_GetDeviceLanguage", FakeJni::JMethodID::PUBLIC},
