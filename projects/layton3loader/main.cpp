...
// libll3.so's native entry points. Unlike libll1.so, the shipped APK's Java
// side goes through a libjniproxy.so that forwards Java_com_Level5_LT3R_
// MainActivity_<method> to PLAIN exports named <method> inside libll3.so
// itself; the Switch port (reference/layton3_nx-main) confirmed this and
...
// convention libll1.so's export used -- matches reference/layton3_nx-main's
...
// reference/layton3_nx-main/source/main.c, which hooks the identical symbols
...
// APK layout under gamefiles/layton3/assets/). init_config() chdir's into
// gamefiles/layton3/, so these prepend "assets/" -- same as the Switch/Vita
// ports' asset_path() helper.
//
// Unlike L1/L2, some assets (notably the nazo/ puzzle graphics) ship only
// inside assets/data0, a ZIP with the game files stored under
// "files/assetpacks/.../assets/<rel>" (see data0.c). A loose-file miss falls
// back to that pack before FS_LoadFile/FS_GetLength report the file absent.
// ---------------------------------------------------------------------------
...
// reference/layton3_nx-main/source/main.c (rot_init / rot_blit), which does
...
// reference/layton3_nx-main/source/main.c:429.
// ---------------------------------------------------------------------------
...
int main(int argc, char *argv[])
{
    ...
    // only here); a missing pack just falls back to loose files, matching
    // reference/layton3_nx-main/source/main.c
    data0_mount(asset_path("data0").c_str());

    // Load the main so file
    printf("Loading libll3\n");
    so_module lmain = {};
    uintptr_t addr_lmain = 0x50000000;
    const char *path_lmain = "lib/arm64-v8a/libll3.so";
    if (!load_so_from_file(&lmain, path_lmain, addr_lmain))
    {
        printf("Failed to load libll3.so.\n");
        return 1;
    }
    ...
    // libll3.so. Passing null makes jnivm hand back the "Invalid" class
    ...
    auto activityClass = vm.findClass("com/Level5/LT3R/MainActivity");
    ...
    printf("calling OnLoad from libll3.so\n");
    ...
    while (running)
    {
        ...
        // (same split as reference/layton3_nx-main/source/main.c).
