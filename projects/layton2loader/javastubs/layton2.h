#ifndef __LAYTON2_H__
#define __LAYTON2_H__
...
// instead -- reference/layton2_nx-main/source/jni.c.) There is no system
...
// libll2.so (Professor Layton and Pandora's Box HD, Level-5's custom engine --
// same family as libll1.so/laytonloader) does not use the standard Android API
// surface. It resolves a fixed set of custom native methods by name on its own
// MainActivity class (movie playback, PNG decode, save directory, license
// checks, the software keyboard) instead of AAssetManager/ANativeActivity.
// This mirrors the method list found in the Switch homebrew port's jni.c
// dispatch table (reference/layton2_nx-main/source/jni.c) for the same binary.
//
// The method set is identical to LT1R's with one addition: OS_GetDeviceLanguage,
// which LT1R never calls. See layton2.cpp for why the Switch port added it.
namespace jnivm
{
    namespace com
    {
        namespace Level5
        {
            namespace LT2R
            {
                ...
                {
                public:
                    DEFINE_CLASS_NAME("com/Level5/LT2R/MainActivity")
                    ...
                    // Sourced from config["device"]["language"] here rather than
                    // a system API, since there is no Switch-style setGetSystemLanguage.
